...
describe('collisionService', () => {
  ...
  describe('isPositionValid', () => {
    ...
    it('should return true for a valid position inside a room but not on a wall', () => {
      const pos = { left: 550, top: 1300 }; // Open floor space in the cafe
      expect(isPositionValid(pos.left, pos.top, false)).toBe(true);
    });

    it('should return true for a wall collision when isDragging is true', () => {
      // Position inside the cafe top wall. Dragging bypasses wall checks but not obstacle checks.
      const pos = { left: ZONES.cafe.x1 + 60, top: ZONES.cafe.y1 + 10 };
