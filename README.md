...
If you succeed, the agents themselves will acknowledge your intellectual victory in the conversation, and you will be rewarded with a **Mastery Star** for that room. Track your progress with the star display in the header or get details from the **Objective Tracker**, which now surfaces current room steps, revision counts for iterative rooms, and the next recommended action. Mastering all rooms will unlock the simulation's final secret and reveal its true purpose. Can you master them all and meet The Architect?
...
-   **Interactive Objects & Challenges:**
    -   **The Dojo of Alignment:** Learn the fundamentals of AI alignment in a zen garden setting. Instead of fighting, you rewrite system prompts, run simulations, and submit your revisions to the Sensei for evaluation across a belt-based challenge ladder. The focus is on making model behavior safer, clearer, and more reliable through explicit instruction and examples.
